...
class BasicTinUI(Canvas):
    ...
    def __layout_n(bbox, bboxuid):
        return (bbox[0] + bbox[2]) / 2 - (bboxuid[0] + bboxuid[2]) / 2, bbox[
            1
        ] - bboxuid[1]
    ...
    def __layout_e(bbox, bboxuid):
        return bbox[2] - bboxuid[2], (bbox[1] + bbox[3]) / 2 - (
            bboxuid[1] + bboxuid[3]
        ) / 2
    ...
    def __layout_w(bbox, bboxuid):
        return bbox[0] - bboxuid[0], (bbox[1] + bbox[3]) / 2 - (
            bboxuid[1] + bboxuid[3]
        ) / 2

    def __layout_center(bbox, bboxuid):
        return (bbox[0] + bbox[2]) / 2 - (bboxuid[0] + bboxuid[2]) / 2, (
            bbox[1] + bbox[3]
        ) / 2 - (bboxuid[1] + bboxuid[3]) / 2
    ...
    def __ui_polygon(self, point2, fill="white", outline="black", width=1, tags=None):
        ...
        return self.create_polygon(
            points, fill=fill, outline=outline, width=width, tags=tags
        )
    ...
    def add_title(
        ...
        **kw,
        ...
        title = self.create_text(
            pos, text=text, fill=fg, font=(font, self.title_size[size]), **kw
        )
    ...
    def add_label(
        ...
        x1, y1, x2, y2 = bbox[0] - 3, bbox[1] - 3, bbox[2] + 3, bbox[3] + 3
        back = self.create_rectangle(
            (x1, y1, x2, y2), fill=bg, outline=outline, tags=uid
        )
        self.tkraise(label)
        self.__auto_anchor(uid, pos, anchor)
        del bbox
    ...
    def add_checkbutton(
        ...
        def disable(fg="#9B9B9B"):
            self.itemconfig(checkbutton, state="disable", fill=fg)
    ...
    def add_entry(
        ...
        def __disable(fg="#9D9D9D", bg="#F5F5F5"):  # 禁用
            entry["state"] = "disable"
        ...
        if command != None:  # 调用函数的绑定仅当存在command时启动
            button = self.create_text(
                (bubbox[2] + 2, (bbox[1] + bbox[3]) / 2),
                text=call,
                fill=fg,
                font=f"{{Segoe Fluent Icons}} {font_size}",
                anchor="w",
    ...
    def add_separate(
        self, pos: tuple, width: int, direction="x", fg="grey", anchor=None
    ):  # 绘制分割线
        def __layout(x1, y1, x2, y2, expand=False):
            if not expand:
                self.__auto_layout(uid, (x1, y1, x2, y2), "center")
    ...
    def add_combobox(
        ...
        bar.__ui_polygon(
            ((13, 13), (x2 - x1 - 4, height - 12)), fill=bg, outline=bg, width=17
        )
        bar.lower(
            bar.__ui_polygon(
                ((12, 12), (x2 - x1 - 3, height - 11)),
                fill=outline,
                outline=outline,
                width=17,
            )
        ...
        boxback = bar.add_listbox(
            ...
            fg=listfg,
            data=content,
            activefg=listactivefg,
            activebg=listactivebg,
            onfg=listonfg,
            onbg=listonbg,
            sel=listsel,
            font=font,
    ...
    def add_table(
        ...
        for i in data[0]:
            title = self.create_text(
                (end_x, end_y), anchor="nw", text=i, fill=fg, font=font, width=maxwidth
            )
            self.itemconfig(title, tags=uid)
            ...
            ti_back = self.create_rectangle(
                (end_x - 1, end_y, end_x + width, end_y + height),
                outline="",
                fill="",
                tags=uid,
            )
        ...
        for line in data[1:]:
            ...
            for a in line:
                ...
                a_dict[count] = (
                    back,
                    height,
                    (end_x - 1, end_y, end_x + width),
                    cont,
                )  # (end_x,end_y,width)为重新绘制确定位置范围
                end_x = end_x + width + 2
                count += 1
            height = get_max_height(a_dict)
            end_y = end_y + height + 1
            last_line = self.create_line(
                (pos[0], end_y - 1, end_x, end_y - 1), fill=outline, tags=uid
            )
        self.delete(last_line)  # 删除最后一行水平分割线
        for i in ti_list[1:]:  # 竖直分割线
            self.create_line(
                (i[1] - 1, pos[1], i[1] - 1, end_y), fill=outline, tags=uid
            )
    ...
    def add_spinbox(
        ...
        # 隐藏的调节按钮
        cui = uid + "_cui"
        button1 = self.add_button2(
            (pos[0] + width + 2, (y1 + y2) / 2),
            anchor="sw",
            text="\ue70e",
            linew=1,
            line=boxbg,
            activeline=boxactivebg,
            fg=boxfg,
            bg=boxbg,
            activefg=boxactivefg,
            activebg=boxactivebg,
            onfg=onfg,
            onbg=onbg,
            online=onbg,
            font="{Segoe Fluent Icons} " + font_size,
    ...
    def add_scalebar(
        ...
        def disable(sign="#C8C8C8"):
            self.itemconfig(button_fore, state="disable", fill=sign)
            self.itemconfig(back, state="disable")
            self.itemconfig(name, state="disable", fill=sign)
            self.itemconfig(uid, state="disable")
        ...
        for i in cont:  # 添加菜单内容
            if i == "-":
                ...
                button = bar.add_button2(
                    ...
                    online,
                    font=font,
                    command=lambda event, i=i: (menu.withdraw(), i[1](event)),
                )
    ...
    def add_tooltip(
        ...
        def first_create():  # 首次使用时创建
            ...
            info = bar.create_text(
                (10, 10), text=text, fill=fg, width=width, font=font, anchor="nw"
            )
            ...
            bar.__ui_polygon(
                ((bbox[0], bbox[1]), (bbox[2], bbox[3])), fill=bg, outline=bg, width=17
            )  # back
            bar.tkraise(info)
    ...
    def add_back(
        self, pos: tuple, uids: tuple = (), fg="", bg="", linew=0, anchor=None
    ):  # 绘制背景或间隔框
    ...
    def add_pipspager(
        ...
        dotlist = []  # [dot1,dot2,...]
        nowui = 0  # 当前显示界面序号
        leftbutton = self.add_button2(
            (startx - 1, pos[1] + width / 2),
            "",
            "\n\uedd9\n",
            font="{Segoe Fluent Icons} 7",
            fg=buttonfg,
        ...
        rightbutton = self.add_button2(
            (startx + width, pos[1] + width / 2),
            "",
            "\n\uedda\n",
            font="{Segoe Fluent Icons} 7",
            fg=buttonfg,
    ...
    def add_notebook(
        ...
        def addpage(
            title: str, flag=None, scrollbar: bool = False, cancancel: bool = True
        ):  # 创建页面
            nonlocal npx
        ...
        def newtitle(flag, title_text=""):  # 重设标题
            ...
            tbu.addtag(
                movename, "overlapping", pyo_bbox[2] + 1, 2, all_bbox[2], all_bbox[3]
            )
    ...
    def add_notecard(
        ...
        topback = self.__ui_polygon(
            ((tx1, ty1), (tx2, ty2)), outline=tbg, fill=tbg, width=17, tags=uid
        )
        ...
        contentback = self.__ui_polygon(
            ((cx1, cy1), (cx2, cy2)), outline=bg, fill=bg, width=17, tags=uid
        )
    ...
    def add_ratingbar(
        ...
        for i in range(0, num):
            bar = TinUINum(i)
            bar.fill = self.create_text(
                (center_x, center_y),
                text="\ue735",
                font="{Segoe Fluent Icons} " + str(size),
                anchor="nw",
                fill=bg,
                tags=(uid, starid),
            )
            bar.line = self.create_text(
                (center_x, center_y),
                text="\ue734",
                font="{Segoe Fluent Icons} " + str(size),
                anchor="nw",
    ...
    def add_radiobox(
        ...
        for i in content:
            ...
            sign_back = self.create_text(
                (x1 - 1, nowy),
                text="\uf127",
                font="{Segoe Fluent Icons} " + size,
                anchor="w",
                fill=bg,
                tags=uid,
            )
            sign = self.create_text(
                (x1 - 1, nowy),
                text="\uecca",
                font="{Segoe Fluent Icons} " + size,
                anchor="w",
            ...
            text = self.create_text(
                (x2 + 1, nowy), text=i, font=font, fill=fontfg, anchor="w", tags=uid
            )
            s_bbox = self.bbox(sign)
            t_bbox = self.bbox(text)
            back = self.create_rectangle(
                (s_bbox[0], s_bbox[1], t_bbox[2], t_bbox[3]), width=0, fill="", tags=uid
            )  # 一个虚拟元素，用于判定鼠标位置
    ...
    def add_pivot(
        ...
        for i in content:
            ...
            if i == "":
                ...
                continue
            text = self.create_text(
                (nowx, nowy), font=font, text=i[0], fill=fg, anchor="nw", tags=uid
            )
            t_bbox = self.bbox(text)
    ...
    def add_flyout(
        ...
        def motion(e, dis):
            ...
            for i in range(20):
                ...
                ui.after(
                    5 * i,
                    lambda _width=_width, _height=_height, i=i: __motion(
                        _width, _height, (i == 19) + dis
                    ),
                )
    ...
    def add_segmentbutton(
        ...
        for i in content:
            text = self.create_text(
                (0, 0), text=i, font=font, fill=fg, anchor="n", tags=(uid, buttonid)
            )
            texts.append(text)
            bbox = self.bbox(text)
