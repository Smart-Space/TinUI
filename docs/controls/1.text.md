...
```python
add_labels(pos:tuple, fg='#000000', bg='#fdfdfd', activefg='#1b1b1b', activebg='#f9f9f9', outline='#eeeeee', font=None, widget=True, anchor='nw', command=None)
```

- pos::位置
- fg::文本颜色
- bg::背景色
- activefg::添加按钮响应鼠标时文本颜色
- activebg::添加按钮响应鼠标时背景颜色
- outline::边框颜色
- font::字体
- widget::是否显示添加按钮
- anchor::对齐方向
- command::添加按钮的回调函数，需要返回`标签名[, 删除回调函数]`
...
> funcs
>
> > `add(label:str, oncancel=None)`::添加标签，如果存在删除回调函数即代表可删除，回调函数需要接受一个参数，即标签名。
> >
> > `get()`::获取标签元素列表`[[text, back, line[, cancelt], index],...]`，分别为文本、背景、边框、删除标识符、位置。

![](https://raw.githubusercontent.com/Smart-Space/TinUI/main/image/TinUI标签栏.gif)
