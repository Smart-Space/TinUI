...
> funcs
>
> > `funcs.change_command(new_func)`::为按钮绑定新函数
> >
> > `funcs.disable(fg='#7a7a7a',bg='#cccccc')`::禁用按钮
> >
> > `funcs.active()`::激活按钮
...
- pos::位置
- text::标题文字
- icon::Segoe Fluent Icons符号编号，为None忽略。如果为文件名（即使不存在），则认为是图片（仅限tkinter支持格式）
...
```python
def add_togglebutton(self,pos:tuple,text:str,fg="#1b1b1b",bg="#fbfbfb",line="#CCCCCC",activefg='#1a1a1a',activebg='#f6f6f6',activeline='#cccccc',onfg="#f3f4fd",onbg="#3041d8",online="#5360de",onactivefg="#e3e3eb",onactivebg="#3c4cda",onactiveline="#626ede",linew=1,font=None,command=None,anchor="nw")
```
...
- activefg::响应鼠标进入文本颜色
- activebg::响应鼠标进入背景色
- activeline::响应鼠标进入边框颜色
- onfg::开启状态文本颜色
- onbg::开启状态背景色
- online::开启状态边框颜色
- onactivefg::开启状态响应鼠标进入文本颜色
- onactivebg::开启状态响应鼠标进入背景色
- onactiveline::开启状态响应鼠标进入边框颜色
- linew::边框宽度
- font::字体
