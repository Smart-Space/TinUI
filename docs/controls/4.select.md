...
> funcs
>
> > `funcs.add(text:str,index=-1)`::在指定位置添加新选项，末尾为-1
> >
> > `funcs.get()`::获取`(选定文本,选定序号)`，未选定为`(None,-1)`
> >
...
> >
> > `funcs.add(uixml=True,index=-1)`在指定位置追加新元素，并且返回`add_ui`返回值。`uixml`对应`add_ui`的`content`参数，表示是否需要初始化TinUIXml
> >
> > `funcs.clear()`清除所有元素
> >
...
返回值：items, items_dict, box, funcs, uid
...
```python
add_segmentbutton(self,pos:tuple,fg='#191919',bg='#EDEDED',activefg='#181818',activebg='#E5E5E5',onbg='#FAFAFA',line='#E5E5E5',sign='#3041d8',font='微软雅黑 12',content=('A','B','C'),command=None,anchor='nw')
```
...
- font-字体
- content-内容
- command-回调函数，必须包含一个参数：选中项的文本
- anchor-对齐方向

绘制一个分段按钮。

返回值：back, outline, button, button2, line, texts, funcs, uid
...
> funcs
>
> > `funcs.select(index:int)`::选择指定位置的选项，会触发回调函数

![](https://github.com/Smart-Space/TinUI/raw/main/image/TinUI分段按钮.gif)
...
```python
add_navigation(self,pos:tuple,minwidth=20,maxwidth=150,bg='#FFFFFF',fg='#1A1A1A',activebg='#E9E9E9',activefg='#191919',onbg='#E9E9E9',onfg='#191919',oncolor='#0067C0',font="微软雅黑 14",content=(("\uE790","Color"),("\uE743","Geometry"),("\uED58","Iconography")),widget=True,command=None,anchor="nw")
```
...
返回值：topicon, topback, line, menus, funcs, uid
...
> line::标识符
>
> menus::`[(icon, text, back),...]`元素集合列表
>
> funcs
>
> > `funcs.navigate(index:int)`::选择指定序号的栏目，会触发回调函数

![](https://github.com/Smart-Space/TinUI/raw/main/image/TinUI导航边栏.gif)