...
```python
add_ui(self,pos:tuple,width:int=200,height:int=200,bg='white',scrollbg='#f0f0f0',scrollcolor='#999999',scrollon='#89898b',scrollbar=False,region='man',content=True,anchor='nw')
```
...
- content::是否初始化TinUIXml，`False`则`ui_xml`为`NoneType`
- anchor::对齐方位
...
>
> ui_xml::该BasicTinUI类的TinUIXml绑定，如果`content=False`，则为`NoneType`
...
> funcs
>
> > `funcs.on_expand(func)`::折叠状态改变触发函数，`func`需要接受一个参数`expand:bool`，当前的操作是折叠(`False`)还是展开(`true`)。
