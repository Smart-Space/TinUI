...
>[!IMPORTANT]
>
>TinUI-6.0弃用，TinUI-6.23重新设计并启用。

```python
add_waitbar1(self,pos:tuple,fg='#0078D7',okcolor='#0f7b0f',r=20,anchor='nw')
```
...
返回值：waitbar1, funcs, uid

> waitbar1::该画布对象
>
> funcs
>
> > `funcs.start`::开启动画，现在默认关闭
> >
> > `funcs.stop`::停止动画，并展现当前进度
> >
> > `funcs.progress(int)`::设置内部进度
...
- fg::动画块颜色
- bg::背景色
- okcolor::完成时（进度为100时）的颜色
- anchor::对齐方向

绘制一个带状等待框。

返回值：back, bar, funcs, uid
...
> >
> > `funcs[1]|funcs.disable(sign='#C8C8C8')`::禁用
> >
...
返回值：top, bottom, back, sc, funcs, uid
...
> funcs
>
> > `speed(float)`，滚动速度（0~1），越小动画越明显
> >
> > `step(float)`，一次滚动幅度（0~1）
> >
> > `moveto(target)`，动画滚动到目标位置（0~1）
...
返回值：bars, back, outline, funcs, uid
